...
const ProductDetail: React.FC<ProductDetailProps> = ({ products, testers = [], onUpdateProduct, showAiInsights }) => {
  ...
  const handleDeleteVersion = (versionToDelete: string) => {
    if (window.confirm(`Are you sure you want to delete ALL records for Version ${versionToDelete}? This cannot be undone.`)) {
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden min-h-[400px]">
        <div className="flex items-center gap-8 px-6 border-b border-slate-100 overflow-x-auto bg-slate-50/50">
          {versions.map((version) => (
            <button key={version} onClick={() => setSelectedVersion(version)} className={`py-4 text-sm font-medium border-b-2 transition-all whitespace-nowrap px-4 ${selectedVersion === version ? 'border-intenza-600 text-intenza-600' : 'border-transparent text-slate-400 hover:text-slate-600 hover:border-slate-200'}`}>
              {version}
            </button>
          ))}
        </div>
          ...
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-2xl font-bold text-slate-800 flex items-center gap-3">
              ...
              <span className={`text-xs font-normal text-white px-2 py-1 rounded-md uppercase tracking-wider ${selectedVersion === product.currentVersion ? 'bg-slate-900' : 'bg-slate-400'}`}>
                {selectedVersion === product.currentVersion ? 'Current' : 'Archived'}
              </span>
            </h3>
            <button 
                onClick={() => onDeleteVersion(selectedVersion)} 
                className="flex items-center gap-1.5 text-xs font-bold text-red-500 hover:bg-red-50 px-3 py-1.5 rounded-lg transition-all border border-transparent hover:border-red-100"
            >
...
const ErgoSection = ({ product, testers, onUpdateProduct, highlightedFeedback }: any) => {
  ...
  return (
    <div className="animate-fade-in space-y-10">
      <div className="flex items-center justify-between"><h2 className="text-xl font-bold text-slate-900">Ergonomics & Human Factors</h2></div>
      <div className="space-y-6">
        <h3 className="text-sm font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2"><ClipboardList size={16}/> Evaluation Projects ({product.ergoProjects.length})</h3>
        {product.ergoProjects.map((project: ErgoProject) => (
           <ProjectCard key={project.id} project={project} testers={testers} />
        ))}
      </div>
      <div className="space-y-6">
        <h3 className="text-sm font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2"><MessageSquare size={16}/> Customer Feedback ({product.customerFeedback.length})</h3>
        ...
        {product.durabilityTests.map((test: TestResult) => (
          <div key={test.id} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm group relative">
            <div className="flex justify-between items-start mb-4">
              <div><span className="text-[10px] font-bold text-intenza-600 uppercase tracking-widest bg-intenza-50 px-2 py-0.5 rounded border border-intenza-100">{test.category}</span><h4 className="font-bold text-slate-800 mt-2">{t(test.testName)}</h4></div>
              <div className={`px-2 py-0.5 rounded text-[10px] font-bold border ${test.status === TestStatus.PASS ? 'bg-green-100 text-green-800 border-green-200' : test.status === TestStatus.FAIL ? 'bg-red-100 text-red-800 border-red-200' : 'bg-slate-100 text-slate-600 border-slate-200'}`}>{test.status}</div>
            </div>
            <div className="flex items-center justify-between mb-2"><span className="text-[10px] font-bold text-slate-400 uppercase">Progress</span><span className="text-sm font-bold text-slate-900">{test.score}%</span></div>
            <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden mb-4"><div className="h-full bg-intenza-500 transition-all duration-1000" style={{ width: `${test.score}%` }}></div></div>
            <div className="flex items-center gap-4 text-[10px] text-slate-400 font-medium"><div className="flex items-center gap-1"><Clock size={12}/> {test.updatedDate}</div></div>
            <div className="absolute top-4 right-4 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity"><button onClick={() => onEditTest(test)} className="p-1.5 bg-slate-50 rounded-lg text-slate-400 hover:text-slate-900 border border-slate-100"><Pencil size={14}/></button><button onClick={() => onDeleteTest(test.id)} className="p-1.5 bg-red-50 rounded-lg text-red-400 hover:text-red-600 border border-red-100"><Trash2 size={14}/></button></div>
          </div>
        ))}
      </div>
    </div>
...
// Fix: Use React.FC to correctly handle reserved 'key' prop when components are used in maps
const ProjectCard: React.FC<{ project: ErgoProject, testers: Tester[] }> = ({ project, testers }) => {
    const { t } = useContext(LanguageContext);
    return (
        <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
            <div className="flex justify-between items-start mb-4">
                <h4 className="text-lg font-bold text-slate-900">{t(project.name)}</h4>
                <span className={`px-2 py-0.5 text-xs font-bold rounded-full ${project.overallStatus === ProjectOverallStatus.PASS ? 'bg-green-100 text-green-800' : project.overallStatus === ProjectOverallStatus.NG ? 'bg-red-100 text-red-800' : 'bg-slate-100 text-slate-600'}`}>{project.overallStatus}</span>
            </div>
            <div className="flex flex-wrap gap-2">
                {project.testerIds.map(tid => {
                    const tester = testers.find((ts: Tester) => ts.id === tid);
                    return tester ? (
...
// Fix: Use React.FC to correctly handle reserved 'key' prop when components are used in maps
const CustomerFeedbackCard: React.FC<{ feedback: ErgoFeedback }> = ({ feedback }) => {
    const { t } = useContext(LanguageContext);
    return (
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
            ...
            <div className="flex items-center justify-between">
                ...
                <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${feedback.status === 'PENDING' ? 'bg-amber-100 text-amber-600' : 'bg-slate-100 text-slate-600'}`}>{feedback.status}</span>
            </div>
        </div>
    );
};
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-fade-in">
       <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden animate-slide-up flex flex-col max-h-[90vh]">
          ...
          <div className="p-6 overflow-y-auto space-y-6">
             {error && (<div className="p-3 bg-red-50 border border-red-200 text-red-600 text-xs font-bold rounded-lg flex items-center gap-2"><AlertCircle size={14}/> {error}</div>)}
             ...
             <div className="grid grid-cols-2 gap-6">
                <div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Target Version (Custom)</label><input type="text" value={formData.version} onChange={e => setFormData({...formData, version: e.target.value})} className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg outline-none focus:ring-2 focus:ring-intenza-500/20 text-sm font-bold" placeholder="e.g. v2.5" /></div>
                <div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Status</label>
                   <select value={formData.status} onChange={e => setFormData({...formData, status: e.target.value as EcoStatus})} className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg outline-none focus:ring-2 focus:ring-intenza-500/20 text-sm font-bold">
             ...
             {formData.status === EcoStatus.IN_PRODUCTION && (
                 <div className="animate-fade-in"><label className="block text-xs font-bold text-intenza-600 uppercase mb-1 flex items-center gap-1"><Calendar size={14} /> Implementation / Ship Date (Required)</label><input type="date" value={formData.implementationDate} onChange={e => setFormData({...formData, implementationDate: e.target.value})} className="w-full px-4 py-2 bg-intenza-50/50 border border-intenza-200 rounded-lg outline-none focus:ring-2 focus:ring-intenza-500/20 text-sm font-bold" /></div>
             )}
             ...
             <div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Attachments</label>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-fade-in">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-slide-up">
        <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-white sticky top-0 z-10"><h2 className="text-xl font-bold">{test ? 'Edit Test' : 'Add Test Result'}</h2><button onClick={onClose} className="p-2 text-slate-400 hover:bg-slate-100 rounded-full"><X size={20}/></button></div>
        <div className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Score (%)</label><input type="number" value={formData.score} onChange={e => setFormData({...formData, score: Number(e.target.value)})} className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg outline-none focus:ring-2 focus:ring-intenza-500/20 text-sm" /></div>
            <div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Status</label><select value={formData.status} onChange={e => setFormData({...formData, status: e.target.value as TestStatus})} className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg outline-none focus:ring-2 focus:ring-intenza-500/20 text-sm font-bold">{Object.values(TestStatus).map(s => <option key={s} value={s}>{s}</option>)}</select></div>
          </div>
          <div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Details</label><textarea value={formData.details[language]} onChange={e => setFormData({...formData, details: {...formData.details, [language]: e.target.value}})} className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg outline-none focus:ring-2 focus:ring-intenza-500/20 text-sm resize-none" rows={3} /></div>
        </div>
        <div className="p-6 border-t border-slate-100 bg-slate-50 flex justify-end gap-3 sticky bottom-0"><button onClick={onClose} className="px-5 py-2 text-slate-600 font-bold hover:bg-slate-200 rounded-lg">Cancel</button><button onClick={() => onSave(formData)} className="px-5 py-2 bg-slate-900 text-white font-bold rounded-lg shadow-lg">Save Test</button></div>
      </div>
    </div>
  );
};
