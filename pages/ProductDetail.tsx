                                                    ...
                                                    {!isPass && (
                                                        <div className="space-y-2">
                                                            ...
                                                            <div className="flex items-center justify-between pt-2 border-t border-slate-50">
                                                                ...
                                                                <button 
                                                                    onClick={() => !isViewer && onStatusClick(project.id, cat, task.id, tid, ngReason?.decisionStatus || NgDecisionStatus.PENDING, ngReason?.linkedEcoId)}
                                                                    disabled={isViewer}
                                                                    className={`px-2 py-0.5 rounded text-[8px] font-bold uppercase border transition-colors ${ngDecisionStyles[ngReason?.decisionStatus || NgDecisionStatus.PENDING]} ${isViewer ? 'cursor-default' : ''}`}
                                                                >
...
// Design Section
const DesignSection = ({ product, shipments, userRole, onAddEco, onEditEco, onDeleteEco, onDeleteVersion, onSetCurrentVersion, onNoShipment }: { product: ProductModel, shipments: ShipmentData[], userRole?: string, onAddEco: () => void, onEditEco: (eco: DesignChange) => void, onDeleteEco: (id: string) => void, onDeleteVersion: (version: string) => void, onSetCurrentVersion: (version: string) => void, onNoShipment: (version: string) => void }) => {
  const { t, language } = useContext(LanguageContext);
  const navigate = useNavigate();
  const isViewer = userRole === 'viewer';
...
// Ergonomics Section
const ErgoSection = ({ product, testers, onUpdateProduct, highlightedFeedback, isFeedbackPanelOpen, setIsFeedbackPanelOpen, userRole }: { product: ProductModel, testers: Tester[], onUpdateProduct: (p: ProductModel) => void, highlightedFeedback: any, isFeedbackPanelOpen: boolean, setIsFeedbackPanelOpen: (o: boolean) => void, userRole?: string }) => {
  const { t, language } = useContext(LanguageContext);
  const navigate = useNavigate();
  ...
  const handleEditProject = (name: LocalizedString, selectedTesterIds: string[]) => {
      if (isViewer || !editingProject) return;
  ...
  const handleOpenEditProject = (project: ErgoProject) => { if (isViewer) return; setEditingProject(project); setStartEvaluationModalOpen(true); };
  const handleDeleteProject = (projectId: string) => { if (isViewer) return; onUpdateProduct({ ...product, ergoProjects: product.ergoProjects.filter(p => p.id !== projectId) }); };
  ...
  const handleSetNgReason = (reason: LocalizedString, isNewTag: boolean, attachments: string[], type: 'ISSUE' | 'IDEA') => {
      if (isViewer || !ngReasonModalState.context) return;
