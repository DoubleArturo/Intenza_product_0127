          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div className="flex items-start gap-6">
              <div className="w-24 h-24 rounded-xl overflow-hidden shadow-md bg-slate-100 flex items-center justify-center text-slate-300">
                 {product.imageUrl ? <img src={product.imageUrl} className="w-full h-full object-cover" /> : <ImageIcon size={40} />}
              </div>
              <div>
          ...
          <div className="lg:col-span-3 space-y-6">
             {activeTab === 'DESIGN' && (
               <DesignSection 
                 ...
                 onDeleteEco={(id) => onUpdateProduct({...product, designHistory: product.designHistory.filter(h => h.id !== id)})} 
                 onSetCurrentVersion={(v) => onUpdateProduct({...product, currentVersion: v})}
               />
             )}
             {activeTab === 'ERGO' && <div ref={ergoSectionRef} className="p-10 bg-white border border-slate-200 rounded-3xl text-center text-slate-400">Ergo module loaded.</div>}
             {activeTab === 'LIFE' && <div className="p-10 bg-white border border-slate-200 rounded-3xl text-center text-slate-400">Durability module loaded.</div>}
          </div>
          <div className="space-y-6 sticky top-40">
             {showAiInsights && <GeminiInsight context={`Analyzing ${product.sku}`} data={product} />}
          </div>
        </div>
      </div>
  ...
  return (
    <div className="animate-fade-in">
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={onAddEco} className="flex items-center gap-2 text-sm bg-slate-900 text-white px-4 py-2 rounded-lg font-medium hover:bg-slate-800 transition-colors shadow-sm"><Plus size={16} /> Add ECO</button>
      </div>
          ...
          <div className="flex items-center justify-between mb-8 border-b border-slate-50 pb-6">
             <div className="flex items-center gap-4">
                ...
                <button 
                  ...
                  title="View customer distribution for this version"
                >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md animate-fade-in">
            <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-2xl animate-slide-up overflow-hidden">
                <div className="flex justify-between items-center p-8 border-b border-slate-100">
                    <h2 className="text-2xl font-bold">{eco ? 'Edit ECO Record' : 'Add New ECO'}</h2>
                ...
                <form onSubmit={(e) => { e.preventDefault(); onSave({...formData, description: { en: formData.description, zh: formData.description }, affectedBatches: [], affectedCustomers: []}); }} className="p-8 space-y-6 max-h-[75vh] overflow-y-auto">
                    <div className="grid grid-cols-2 gap-6">
                        <div><label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Target Version</label><input required className="w-full px-4 py-2.5 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:border-slate-900 transition-all font-bold" value={formData.version} onChange={e => setFormData({...formData, version: e.target.value})} /></div>
                        <div><label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Status</label><select className="w-full px-4 py-2.5 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:border-slate-900 transition-all font-bold" value={formData.status} onChange={e => setFormData({...formData, status: e.target.value as EcoStatus})}>{Object.values(EcoStatus).map(s => <option key={s} value={s}>{s}</option>)}</select></div>
                    </div>
                    <div><label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Description</label><textarea required className="w-full px-4 py-3 bg-slate-50 border-2 border-slate-100 rounded-2xl resize-none focus:border-slate-900 transition-all font-medium" rows={3} value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} /></div>
                    
                    {/* NEW MULTI-IMAGE UPLOAD AREA */}
                    <div>
                        <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Supporting Documentation / Images</label>
                        <div className="flex flex-wrap gap-3">
                            {formData.imageUrls.map((url: string, i: number) => (
                                <div key={i} className="relative w-20 h-20 rounded-2xl border border-slate-200 overflow-hidden group">
                                    ...
                                    <button type="button" onClick={() => setFormData({...formData, imageUrls: formData.imageUrls.filter((u:string)=>u!==url)})} className="absolute inset-0 bg-red-500/80 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"><Trash2 size={16}/></button>
                                </div>
                            ))}
                            <button type="button" onClick={() => fileInputRef.current?.click()} disabled={isUploading} className="w-20 h-20 rounded-2xl border-2 border-dashed border-slate-200 flex flex-col items-center justify-center text-slate-400 hover:border-slate-400 hover:text-slate-600 bg-slate-50 transition-all">
                    ...
                    <div className="flex gap-4 pt-6">
                        <button type="button" onClick={onClose} className="flex-1 py-4 text-slate-500 font-bold hover:bg-slate-50 rounded-2xl transition-all uppercase tracking-widest">Cancel</button>
                        <button type="submit" disabled={isUploading} className="flex-1 py-4 bg-slate-900 text-white font-bold rounded-2xl hover:bg-slate-800 transition-all shadow-xl shadow-slate-900/30 disabled:opacity-50 uppercase tracking-widest">Save Record</button>
                    </div>
                </form>
            </div>
        </div>
